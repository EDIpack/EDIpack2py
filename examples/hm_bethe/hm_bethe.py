...
while not converged and iloop < ed.Nloop:
    ...
    if rank == 0:
        np.savetxt(
            "Gloc_iw.dat",
            np.transpose([wm, Gmats[0, 0, 0, 0, :].imag, Gmats[0, 0, 0, 0, :].real]),
        )
        np.savetxt(
            "Gloc_realw.dat",
            np.transpose([wr, Greal[0, 0, 0, 0, :].imag, Greal[0, 0, 0, 0, :].real]),
        )
    ...
    if rank == 0:
        np.savetxt(
            "Delta_iw.dat",
            np.transpose([wm, Delta[0, 0, 0, 0, :].imag, Delta[0, 0, 0, 0, :].real]),
        )
